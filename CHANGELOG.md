...
### Changed
- **Keyboard up now means a pure ride view.** v0.5.4's thin readout strip goes too: while typing on a phone the whole cab steps aside and the map takes everything between the top edge and the LED board (~320px on a typical phone, up from ~230). The counters keep running behind the scenes, and the full cab returns the instant the keyboard closes. Implementation note for future edits: the cab collapses via `height:0` + `visibility:hidden`, deliberately **not** `display:none` — removing a `#game` grid child shifts the map and board up one row, dropping the board into the collapsible `1fr` row where it gets crushed to its padding (this bit during development).
- **The ride map wears bigger furniture on touch screens.** Phones and tablets render the train, station dots, heat rings, line strokes and station names 1.25× their desktop size at every camera depth: the v0.4.16 counter-scale now carries a per-device factor (`GM_TOUCH`, from `pointer:coarse`), so the map reads chunkier under a thumb without changing the desktop look. The completion burst, bend rounding and the terminus recap's label-fit math all follow the same factor automatically; the menu map is untouched.
...
### Added
- **On phones, the game now tucks itself above the soft keyboard.** Opening the keyboard used to simply cover the bottom half of the screen — including the LED board with the very word you're supposed to type (the game screen is a fixed full-viewport grid, so there was nothing to scroll either). Now, whenever the keyboard is up (detected via `visualViewport`: the visual viewport squeezed well below the layout viewport while the typing sink holds focus), the cab compresses into the strip that stays visible: the map row gives way first, and the station name + pinyin ride directly above the keyboard. On screens too small even for HUD + board (short phones, landscape), the screen scrolls internally with the board pinned at the bottom — the word always wins. Everything relaxes back the moment the keyboard closes; desktop is untouched (a window resize shrinks both viewports together, so the detection never trips). One subtlety worth recording: the board row is sized `max-content` while compressed, because `#board` is an `overflow:hidden` panel whose automatic grid minimum is zero — a plain `auto` row would crush it to its padding under extreme squeeze and spill the word invisibly past the screen.
...
### Changed
- **Going down to the level picker is now a one-way trip — the home screen hands over cleanly instead of lingering above.** Pressing 选择关卡 (or scrolling all the way past the hero) folds the title screen out of the page entirely: the network map and line cards become the top of the page, so scrolling up can never wander back into the hero — there is simply nothing above to scroll to. The ↑ 首页 button (bottom-right, as before) is the one way home, unfolding the title screen back in from the top. Returning from a run or the result screen lands directly on the picker in the same state. The fold/unfold animates with the usual MD3 easing and is instant under reduced motion; scrolling within the picker (cards, leaderboard) is untouched.
...
### Changed
- **The completion burst is a little bolder, and the name floats clear of the train.** The green ring grows slightly (radius 9→11, interchanges 13→15, stroke 2.5→3) and the station name steps up from 14px to 17px with a touch more outline — reading as a proper reward rather than a blip at the deep ride zoom. The name also sits ~60% farther above the platform (21→34 units, interchanges 26→40), so it no longer crowds the train parked on the dot, and its entrance float is a hair livelier.
...
### Changed
- **Softer city and district outlines in light mode.** The Guangzhou/Foshan city boundaries and Guangzhou district lines introduced in v0.4.9 read a little heavy against the light theme's cream surface; they're now lightened so they recede further into the background and the metro lines lead more clearly. The dark theme is unchanged.
...
### Fixed
- **Lines on the menu map are clickable again.** v0.4.7's pan/zoom captured the pointer the instant you pressed down, which retargeted the follow-up click away from the line you were pressing — so tapping a line on the map did nothing. The map now captures the pointer only once a drag or pinch actually begins, so a plain click still selects (or a drag still pans, without misfiring a selection).
